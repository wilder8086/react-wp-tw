          ...
          {tasks.map((task) => (
            <div className="bg-gray-900 px-20 py-5 text-white shadow-2x1 mb-4 flex justify-between" key={task.id}>
              <div className="text-left">
                ...
                <button className="bg-purple-600 hover:bg-purple-500 py-1 px-3 mt-2 rounded" onClick={() => toggleTaskDone(task.id)}>
                  {task.done ? "Undone" : "Done"}
                </button>
              </div>
              <div>
                <Link to={`/edit/${task.id}`} className="bg-gray-600 hover:bg-gray-500 py-2 px-4 mr-2 rounded">Edit</Link>
                <button className="bg-red-600 hover:bg-red-500 py-2 px-4 mr-2 rounded" onClick={() => deleteTask(task.id)}>Delete</button>
              </div>
            </div>
          ))}
        </div>
